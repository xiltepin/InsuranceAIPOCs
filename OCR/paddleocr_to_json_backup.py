...
class InsuranceDocumentExtractor:
    def preprocess_image(self, image_path):
        """Minimal preprocessing for speed - only resize if too large."""
        from PIL import ImageEnhance
        img = Image.open(image_path)
        
        # Only resize if image is very large (>2000px width)
        if img.width > 2000:
            ...
            # Use original image if size is reasonable
            return image_path
    def __init__(self, languages=['en'], use_gpu=True):
        ...
        try:
            ...
            self.ocr = PaddleOCR(
                lang=languages[0],
                use_gpu=use_gpu,
                gpu_mem=2000,  # Limit GPU memory usage to 2GB for efficiency
                det_model_dir=None,  # Use default models but with optimization
                rec_model_dir=None,
                cls_model_dir=None,
                det_max_side_len=960,  # Reduce from default 1536 for speed
                rec_batch_num=8,  # Increase batch size for GPU efficiency
                show_log=False  # Disable verbose logging for speed
            )
            print(f"[INFO] PaddleOCR initialized with speed optimizations", file=sys.stderr)
        except Exception as e:
            # Fallback to basic configuration if optimized fails
            print(f"[WARN] Optimized init failed, using basic config: {e}", file=sys.stderr)
            self.ocr = PaddleOCR(lang=languages[0])
            print(f"[INFO] PaddleOCR initialized with basic configuration", file=sys.stderr)
    ...
    def create_enhanced_prompt(self, raw_text, image_path):
                ...
                prompt = f'''Parse insurance data to JSON:
{raw_text[:1000]}
...
JSON format:
        ...
        try:
            ...
            payload = {
                ...
                "options": {
                    ...
                    "top_p": 0.8,  # Reduce top_p for faster token selection
                    "num_predict": 1500,  # Further reduce tokens for speed (insurance forms are predictable)
                    "num_ctx": 2048,  # Reduce context window for speed
                    "num_batch": 2048,  # Maximize batch size for GPU throughput
                    "num_gpu": 99,  # Force maximum GPU layers (Tesla P100 can handle full model)
                    "num_thread": 1,  # Minimize CPU threads - let GPU do the work
                    "repeat_penalty": 1.05,  # Reduce penalty for faster processing
                    "top_k": 20,  # Limit top-k for faster token selection
                    "stop": ["```", "---", "END_JSON", "}\n\n"]  # More stop tokens for faster completion
                }
            }
            ...
            # Fast non-streaming response parsing
            print("[PROGRESS] Fast GPU processing...", file=sys.stderr)
            ...
            if not content:
                print(f"[ERROR] No content in response! Full response: {response_data}", file=sys.stderr)
    ...
    def process_document(self, image_path):
        ...
        if extracted_data:
            ...
            # Add text blocks for frontend display
    ...
    if "--raw-text" in sys.argv:
        try:
            ...
            if raw_text_index + 1 < len(sys.argv):
                ...
                # Create text blocks from raw text lines for frontend display
                ...
                for i, line in enumerate(lines):
                    if line.strip():  # Only include non-empty lines
                        text_block = {
                            "text": line.strip(),
                            "confidence": 1.0,  # Raw text files have 100% "confidence"
                            "bbox": f"Line {i+1}"
                        }
                        processed_data["text_blocks"].append(text_block)
                
                # Set raw_ocr_text for compatibility
